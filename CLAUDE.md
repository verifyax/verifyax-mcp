...
- **The skill stays the canonical API reference.** When the API changes, update `docs/verifyax-api.md` first (and the skill in the `verifyax-plugins-claude` repo), then update the SDK, then the MCP server.

## Quick context for new Claude Code sessions

If you're reading this fresh: we have a working API skill in production at `verifyax/verifyax-plugins-claude`. It teaches Claude how to drive the VerifyAX API via Python scripts. This repo is the next layer — turning that same capability into native MCP tools so Claude can execute the workflow directly instead of writing scripts for the user to run.
